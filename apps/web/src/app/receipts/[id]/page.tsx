...
export default function ReceiptPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const data = await publicApi<Receipt>(`/receipts/${encodeURIComponent(id)}`);
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center gap-2 text-sm text-[color:var(--pay-muted)]">
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-2xl px-3 py-8 sm:px-4 sm:py-10">
      <div className="overflow-hidden rounded-2xl border border-[color:var(--pay-border)] bg-white shadow-panel print:shadow-none">
        <div className="flex flex-col gap-4 border-b border-[color:var(--pay-border)] bg-gradient-to-br from-navy via-navy to-navy-700 px-4 py-5 text-white sm:flex-row sm:items-start sm:justify-between sm:gap-4 sm:px-6 sm:py-6">
        ...
        <div className="space-y-5 px-4 py-5 sm:px-6 sm:py-6">
          <div className="rounded-xl bg-[color:var(--surface)] p-4">
            <p className="text-xs uppercase tracking-wide text-[color:var(--pay-muted)]">Amount paid</p>
          ...
          {receipt.qrVerification ? (
            <div className="rounded-lg border border-[color:var(--pay-border)] p-4 text-center">
          ...
          {receipt.digitalSignature ? (
            <div className="rounded-lg border border-[color:var(--pay-border)] p-3">
              <p className="text-xs uppercase tracking-wide text-[color:var(--pay-muted)]">Digital signature</p>
              <p className="mt-1 break-all font-mono text-[11px] text-[color:var(--pay-muted)]">
                {receipt.digitalSignature}
              </p>
            </div>
          ) : null}
          ...
          <div className="flex flex-col gap-2 print:hidden sm:flex-row sm:flex-wrap">
            ...
            <button
              ...
              className="inline-flex h-10 w-full items-center justify-center rounded-md border border-[color:var(--pay-border)] px-4 text-sm sm:w-auto"
            >
              New payment
            </button>
          </div>
