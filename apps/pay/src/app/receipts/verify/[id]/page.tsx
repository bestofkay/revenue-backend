...
export default function VerifyReceiptPage() {
    ...
    (async () => {
      try {
        const [rec, key] = await Promise.all([
          ...
          api<{ publicKey: string }>('/receipts/public-key'),
        ]);
        if (cancelled) return;
        setReceipt(rec);
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-lg flex-col justify-center px-3 py-8 sm:px-4 sm:py-10">
      <div className="rounded-2xl border border-[color:var(--border)] bg-white p-4 shadow-panel sm:p-6">
        {status === 'loading' && (
        ...
        {status === 'valid' && (
          <div className="space-y-3">
            ...
            <p className="break-words text-sm text-[color:var(--muted)]">{detail}</p>
            {receipt && (
        ...
        <button type="button" className="mt-6 text-sm text-accent underline" onClick={() => router.push('/')}>
          Back to pay portal
        </button>
      </div>
    </div>
